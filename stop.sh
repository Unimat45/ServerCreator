...
while [ $CountdownTime -gt 0 ]; do
  if [ $CountdownTime -eq 1 ]; then
    screen -Rd $pathName -X stuff "say Restarting server in 60 seconds...$(printf '\r')"
    sleep 30;
    screen -Rd $pathName -X stuff "say Restarting server in 30 seconds...$(printf '\r')"
    sleep 20;
    screen -Rd $pathName -X stuff "say Restarting server in 10 seconds...$(printf '\r')"
    sleep 10;
  else
    screen -Rd $pathName -X stuff "say Restarting server in $CountdownTime minutes...$(printf '\r')"
    sleep 60;
  fi
  echo "Waiting for $CountdownTime more minutes ..."
done
echo "Restarting Minecraft server ..."
screen -Rd $pathName -X stuff "say Restarting server (stop.sh called)...$(printf '\r')"
...
echo "Minecraft server $pathName stopped."
